...
import DeleteSnapshotForm from './DeleteSnapshotForm';
...
describe('DeleteSnapshotForm', () => {
  ...
  test('renders form fields', () => {
    render(<DeleteSnapshotForm setActivity={mockSetActivity} showFeedback={mockShowFeedback} />);
  ...
  test('submits form successfully with access token', async () => {
    ...
    axios.delete.mockResolvedValueOnce({ data: { message: 'Snapshot deleted successfully' } });
    ...
    await waitFor(() => {
      ...
      expect(axios.delete).toHaveBeenCalledWith('/delete-snapshot', {
        data: {
      ...
      expect(mockShowFeedback).toHaveBeenCalledWith('Snapshot deleted successfully', 'success');
    });
  });
  ...
  test('handles API error', async () => {
    const mockToken = 'mock-access-token';
    mockAcquireTokenSilent.mockResolvedValueOnce({ accessToken: mockToken });
    const errorMessage = 'An error occurred while deleting the snapshot';    axios.delete.mockRejectedValueOnce( { data: { error: errorMessage } });
    ...
    await waitFor(() => {
      expect(mockAcquireTokenSilent).toHaveBeenCalled();
      expect(axios.delete).toHaveBeenCalledWith('/delete-snapshot', expect.any(Object));
      expect(mockSetActivity).toHaveBeenCalledWith(expect.any(Function));
